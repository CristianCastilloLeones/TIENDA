 ...
 *
  * @author cl
 */
public class GenradordeItemsController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        venta.setOnKeyPressed((KeyEvent event) -> {
        ...
        ELIMINAR.setOnAction((event) -> {
            ObservableList<recepcionpago> selectedIndices = tabladetalle.getSelectionModel().getSelectedIndices();
        });
        ...
        nombrecliente.setOnKeyPressed((KeyEvent event) -> {
            if (!nombrecliente.getText().replace(" ", "").isEmpty()) {
                if (event.getCode() == KeyCode.ENTER) {
                    ...
                    idS();
                     BuscarFacturacionExistente(true);
                }
            }
        });
        ...
        detalle1.setOnKeyPressed((KeyEvent event) -> {
            ...
            if (event.getCode() == KeyCode.ENTER) {
                if (onVariable.getList().contains(detalle1.getText())) {
                    BuscarFacturacionExistente(false);
    ...
    public void buscarValorMensual() {
        ...
        try {
            ...
            Logger.getLogger(GenradordeItemsController.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public void BuscarFacturacionExistente(boolean Valor) {
       String X="";
        ...
        try {
            while (q.auxConsulta.next()) {
                if (Verificador(detalle1.getText().replace(" ", "").toUpperCase(), q.auxConsulta.getString("detalle").replace(" ", "").toUpperCase())) {
                 ...
                 X=X + q.auxConsulta.getString("detalle").trim()+"\n";
            }
            if(Valor){
                Alestas(" HITORIAL DE FACTURACION \n" + X);
            }
        } catch (SQLException ex) {
            Logger.getLogger(GenradordeItemsController.class.getName()).log(Level.SEVERE, null, ex);
        }
